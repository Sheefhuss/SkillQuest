...
export default function Missions() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      const aiMission = await apiClient.post("/ai/mission", { profile: me, week_start: ws });
  ...
  const toggleObjective = async (idx) => {
    const updated = current.objectives.map((o, i) =>
      i === idx ? { ...o, completed: !o.completed, progress: !o.completed ? o.target : 0 } : o
    );
    const allDone = updated.every((o) => o.completed);
    await apiClient.patch(`/missions/${current.id}`, { objectives: updated, completed: allDone });
  ...
  return (
    <div className="space-y-6">
      <div>
        <div className="text-xs uppercase tracking-[0.3em] text-muted-foreground">AI personalised</div>
        <h1 className="font-display text-4xl md:text-5xl mt-1">Weekly <span className="text-gradient">Missions</span></h1>
        <p className="text-muted-foreground mt-2">Your AI strategist tailors a weekly quest to your skill level.</p>
      </div>
        ...
        <GlassCard className="p-8 text-center space-y-4 neon-border">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1 max-w-sm mx-auto">
              AI-personalised weekly missions are exclusive to Pro members. Upgrade to get a quest tailored to your exact skill level every week.
            </p>
          </div>
          <div className="text-left max-w-xs mx-auto space-y-2 text-sm text-muted-foreground">
            {["Personalised to your weak spots", "New mission every Monday", "Earn bonus XP on completion", "Tracked progress & objectives"].map((f) => (
        ...
        <>
          {!current ? (
            <GlassCard className="p-10 text-center">
              ...
              <div className="font-display text-2xl">No mission for this week yet</div>
              <p className="text-muted-foreground mt-1">Let the AI craft your next quest.</p>
              <Button
                onClick={generate}
                disabled={generating}
              ...
              <GlassCard className="p-6 md:p-8 relative overflow-hidden neon-border">
                ...
                <div className="relative">
                  <div className="flex items-center gap-2 text-xs uppercase tracking-widest text-accent">
                    <Target className="w-3.5 h-3.5" /> {current.difficulty} · +{current.xp_reward} XP
                  </div>
                  <h2 className="font-display text-3xl md:text-4xl mt-2">{current.title}</h2>
                  <p className="text-muted-foreground mt-2 max-w-2xl">{current.narrative}</p>
                  <div className="mt-6 space-y-3">
                    {current.objectives?.map((o, i) => (
                      <button
                        key={i}
                        onClick={() => toggleObjective(i)}
                        className="w-full text-left flex items-center gap-3 p-3 rounded-xl hover:bg-secondary/50 transition"
                      >
                        <div className={`w-6 h-6 rounded-full grid place-items-center border-2 shrink-0 ${o.completed ? "bg-xp border-xp" : "border-border"}`}>
                          {o.completed && <CheckCircle2 className="w-3.5 h-3.5 text-background" />}
                        </div>
                        <div className="flex-1">
                          <div className={o.completed ? "line-through text-muted-foreground" : ""}>{o.label}</div>
                        </div>
                        <div className="text-xs text-muted-foreground">Target: {o.target}</div>
                      </button>
                    ))}
                  </div>
                  {current.completed && (
                    <div className="mt-6 flex items-center gap-2 text-xp">
                      <Sparkles className="w-4 h-4" /> Mission complete! XP awarded.
                    </div>
                  )}

                  {/* Allow regenerating next week's mission */}
                  {current.completed && (
                    <Button
                      onClick={generate}
                      disabled={generating}
