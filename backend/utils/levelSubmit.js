...
const IDIOM_HINTS = {
  ...
  cpp:        "In C++, STL (std::sort, std::reverse, vectors, maps) are all valid. Do NOT penalise STL usage.",
  javascript: "Enforce challenge-specific restrictions (e.g. no .reverse()) strictly only for JavaScript.",
};
...
async function evaluateLevelChallenge(code, prompt, language = "javascript") {
  ...
  if (evalCache.has(cacheKey)) {
    console.log("[LevelEval] Cache hit");
...
function mountLevelSubmit(app, authenticateToken, User) {
  app.post("/api/levels/:id/submit", authenticateToken, async (req, res) => {
    try {
      ...
      console.error("Level submit error:", err?.message);
